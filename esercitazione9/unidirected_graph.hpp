...
class unidirected_graph
{
    ...
    public:
        ...
        /* METODO add_edge */
        // Modificato rispetto all'esercitazione 8 per evitare edges duplicati
        void add_edge(const unidirected_edge& edge)
        {
            int u = edge.from();
            int v = edge.to();

            // Se l'arco esiste già, NON lo aggiungo di nuovo
            auto it = numerazione_edges.find(edge);
            if (it != numerazione_edges.end()) {
                // Già presente: aggiorno comunque la mappa dei vicini per sicurezza
                nodi_adiacenti[u].insert(v);
                nodi_adiacenti[v].insert(u);
                return;
            }

            // Aggiungo l’arco alla lista degli edge (ordine di inserimento)
            edges_list.push_back(edge);

            // Aggiorno la mappa dei vicini (è un set, quindi niente duplicati qui)
            nodi_adiacenti[u].insert(v);
            nodi_adiacenti[v].insert(u);

            // Calcolo l’indice del nuovo arco (ultimo della lista)
            int index = static_cast<int>(edges_list.size()) - 1;
            numerazione_edges[edge] = index; // Associa all’arco edge il numero index
        }
